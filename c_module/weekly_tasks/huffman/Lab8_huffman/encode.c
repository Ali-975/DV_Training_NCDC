...
void pq_insert(struct tnode* p)
{
   ...
   if(curr==qhead)
   {
        ...
        p->next = qhead; // Insert before the first element
        qhead = p;
   }
   else /*insert between prev and next*/
   {
	 	/*TODO: write code to insert in between*/
        p->next = curr; // Insert in middle or end
        prev->next = p;
   }
}
...
int main()
{
	...
	puts("orginal:abcdabcdabcdefefefefef");
	fout=fopen(OUT_FILE,"w");
	encode("abcdabcdabcdefefefefef",stdout);
	encode("abcdabcdabcdefefefefef",fout);
	fclose(fout);
	// getchar();
