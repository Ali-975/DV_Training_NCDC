...
// =====================================================================
    ...
    control_unit cu(.opcode(if_id_opcode), .reg_we(reg_we), .mem_we(mem_we), 
                .mem_reg_w(mem_reg_w), .alu_op(alu_op), .op_a_sel(op_a_sel),
                .op_b_sel(op_b_sel), .pc_sel(pc_sel), .jump(jump), .branch(branch));
                
    imm_generation imm_gen(.instr(if_id_imm_val), .imm(imm));
    ...
    data_mem dm(.clk(clk), .rst(rst), .mem_re(ex_mem_func_3), 
                .mem_we(ex_mem_mem_we), .d_mem_addr(ex_mem_result), 
...
//    top single_cycle_module(.clk(clk), .rst(rst));
    ...
    if_id_stage IF_ID(.clk(clk), .rst(rst),
                
                .if_id_instr(instr), .if_id_opcode(if_id_opcode), .if_id_rs1(if_id_rs1), 
                .if_id_rs2(if_id_rs2), .if_id_rd(if_id_rd), .if_id_func_3(if_id_func_3), 
                .if_id_func_7(if_id_func_7), .if_id_imm_val(if_id_imm_val), 
                
                .pc(pc), .if_id_pc(if_id_pc)
    );
    
    id_ex_stage ID_EX(.clk(clk), .rst(rst), 
                .reg_out_1(reg_out_1), .reg_out_2(reg_out_2), .if_id_rd(if_id_rd), 
                ...
                .reg_we(reg_we), .mem_we(mem_we), .mem_reg_w(mem_reg_w), 
    ...
    ex_mem_stage EX_MEM(.clk(clk), .rst(rst), 
                ...
                .id_ex_rd(id_ex_rd), .id_ex_reg_out_2(id_ex_reg_out_2), 
                
                .ex_mem_rd(ex_mem_rd), .ex_mem_reg_out_2(ex_mem_reg_out_2),
                
                .id_ex_reg_we(id_ex_reg_we), .id_ex_mem_we(id_ex_mem_we), 
                .id_ex_mem_reg_w(id_ex_mem_reg_w), .id_ex_pc_sel(id_ex_pc_sel), 
                .id_ex_jump(id_ex_jump), .id_ex_branch(id_ex_branch), 
                .id_ex_func_3(id_ex_func_3),
                
                .ex_mem_reg_we(ex_mem_reg_we), .ex_mem_mem_we(ex_mem_mem_we), 
                .ex_mem_mem_reg_w(ex_mem_mem_reg_w), .ex_mem_pc_sel(ex_mem_pc_sel), 
...
//    assign func_3 = if_id_instr[14: 12];
//    assign func_7 = if_id_instr[31: 25];
    
    
    
    
    // for alu operand b
    assign op_b = (id_ex_op_b_sel) ? id_ex_imm : id_ex_reg_out_2;
    
    // for alu operand a
    always_comb begin
        unique case (id_ex_op_a_sel)
            2'b00: op_a = id_ex_reg_out_1;
    ...
    always_comb begin
        if(ex_mem_jump)
            ...
            wb_data = (mem_wb_mem_reg_w) ? mem_wb_d_mem_d_out : mem_wb_result;
    end
    ...
    always_comb begin
            unique case (ex_mem_pc_sel)
                2'b00: next_pc = pc + 4;
                2'b01: next_pc = ex_mem_pc + ex_mem_imm;                        // jal and branch
                2'b10: next_pc = reg_out_1 + ex_mem_imm;                        // jalr
                2'b11: begin                                                    // branch
