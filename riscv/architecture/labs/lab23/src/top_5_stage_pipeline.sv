...
// =====================================================================
    ...
    alu a_l_u(.op_1(op_a), .op_2(op_b), .alu_instr(alu_instr),
                .func_3(id_ex_func_3), .zero(zero), .result(alu_out), .branch_taken(branch_taken) );
...
//    top single_cycle_module(.clk(clk), .rst(rst));
    ...
    if_id_stage IF_ID(.clk(clk), .rst(rst), .stall(stall), .flush(if_id_flush),
    ...
    id_ex_stage ID_EX(.clk(clk), .rst(rst), .stall(stall), .flush(id_ex_flush),
                
                .if_id_opcode(if_id_opcode), .id_ex_opcode(id_ex_opcode),
    ...
    ex_mem_stage EX_MEM(.clk(clk), .rst(rst), 
                ...
                .zero(zero), .result(alu_out), .branch_taken(branch_taken),
...
// ====================================================================================
    ...
    .if_id_opcode(if_id_opcode), .if_id_rs1(if_id_rs1), .if_id_rs2(if_id_rs2),
    
    .id_ex_opcode(id_ex_opcode), .id_ex_rd(id_ex_rd), .id_ex_mem_re(id_ex_mem_re), 
    
    .branch_taken(id_ex_branch & branch_taken), 
    
    .pc(pc), .id_ex_pc(if_id_pc),
...
//    assign func_7 = instr[31: 25];
    ...
    always_comb begin
            unique case (id_ex_pc_sel)
                ...
                2'b01: next_pc = id_ex_pc + id_ex_imm;                // jal 
                2'b10: next_pc = id_ex_reg_out_1 + id_ex_imm;         // jalr
                2'b11: begin
                    if(id_ex_branch & branch_taken)
                        next_pc = id_ex_pc + id_ex_imm;               // branch
                    else
                        next_pc =  pc + 4;
                end
            endcase
